...
export function loadPatterns(patternsDir, cli) {
    const filePath = join(patternsDir, `${cli}.patterns`);
    const content = readFileSync(filePath, 'utf-8');
    return parsePatternsContent(content, cli);
}
export function parsePatternsContent(content, cli) {
    const vars = {};
    ...
    for (const [varName, field] of Object.entries(PATTERN_KEYS)) {
        result[field] = compileOptional(vars[varName]);
    }
    return result;
}
export function listAvailableCLIs(patternsDir) {
...
export function bundledPatternsDir() {
...
export function getPatterns(cli, patternsDir) {
    if (patternsDir) {
        try {
